...
const HotelDetails = ({
  ...
  const { hotel: storeHotel, loading, fetchHotel, setHotel } = useHotelDetailsStore();
  const { favorites, addFavorite, removeFavorite, fetchFavorites } =
    useFavoriteStore();
  const { setBookingData, clearTravelerDetails } = useBookingStore();

  // Try to load basic hotel data from localStorage synchronously for the VERY FIRST render
  const hydratedData = useMemo(() => {
    if (typeof window === "undefined" || !hotelId) return null;
    try {
      const storedData = localStorage.getItem(`hotel_basic_${hotelId}`);
      if (storedData) {
        console.log("Synchronously hydrated hotel data for first render");
        return JSON.parse(storedData);
      }
    } catch (e) {
      console.error("Failed to parse stored hotel data", e);
    }
    return null;
  }, [hotelId]);

  // Use store data if available, otherwise fallback to hydrated data (only after mounting to avoid hydration errors)
  const hotelData = storeHotel || (hasMounted ? hydratedData : null);

  // Set hasMounted to true on mount
  useEffect(() => {
    setHasMounted(true);
  }, []);

  // Sync store with localStorage data on mount if store is empty
  useEffect(() => {
    if (hasMounted && hotelId && !storeHotel && hydratedData) {
      setHotel(hydratedData);
    }
  }, [hasMounted, hotelId, storeHotel, hydratedData, setHotel]);

  // Clean up stored basic data once we have the full details to keep localStorage clean
  useEffect(() => {
    if (hotelData && hotelData.rooms && hotelData.rooms.length > 0 && hotelId) {
      localStorage.removeItem(`hotel_basic_${hotelId}`);
      console.log(`Cleaned up temporary hydration data for hotel ${hotelId}`);
    }
  }, [hotelData, hotelId]);
  ...
  const getOrderedHotelImages = useCallback(() => {
    ...
    const images = (hotelData.images as HotelImage[]).filter((img: HotelImage) => !!img?.path);
    ...
    const genImages = images
      .filter((img: HotelImage) => img.type?.code === "GEN")
  ...
  useEffect(() => {
    if (hotelData && hotelData.rooms) {
      ...
      const roomsWithDetails = hotelData.rooms.map((room: HotelRoom) => {
        ...
        let finalRawImages = (hotelData.images || []).filter(
          (img: HotelImage) => img.roomCode === room.roomCode && img.path
        );
        ...
        if (finalRawImages.length === 0) {
          ...
          const sameTypeRooms = (hotelData.rooms || []).filter((r: HotelRoom) =>
          ...
          if (sameTypeRooms.length > 0) {
            const sameTypeImages = (hotelData.images || []).filter((img: HotelImage) =>
              sameTypeRooms.some((str: HotelRoom) => str.roomCode === img.roomCode) && img.path
            );
        ...
        if (finalRawImages.length === 0) {
          ...
          (hotelData.rooms || []).forEach((otherRoom: HotelRoom) => {
            if (otherRoom.roomCode === room.roomCode) return;
            const otherImages = (hotelData.images || []).filter(
              (img: HotelImage) => img.roomCode === otherRoom.roomCode && img.path
            );
        ...
        const generalHotelImages = (hotelData.images || []).filter((img: HotelImage) => {
        ...
        const roomImages = targetImages
          ...
          .sort((a: ProcessedRoomImage, b: ProcessedRoomImage) => {
        ...
        const facilities = (room.roomFacilities || []).map((facility: Facility) => ({
        ...
        const roomStays = (room.roomStays || []).map((stay: RoomStay) => ({
          type: stay.stayType || "",
          order: stay.order || "",
          description: stay.description || "",
          facilities: (stay.roomStayFacilities || []).map((f: RoomStayFacility) => ({
            code: f.facilityCode,
      ...
      const roomsWithRates = roomsWithDetails.filter(
        (room: ProcessedRoom) => room.rates.length > 0
      );
  ...
  const groupedAmenities = amenities.reduce((acc: Record<number, Facility[]>, amenity: Facility) => {
          ...
          <section id="rooms" className="rooms-filter-section">
            ...
            <div className="hotel-room-with-total">
              <div className="room-list-vertical">
                {(loading || !hasMounted) && processedRooms.length === 0 ? (
                  <div className="loading-rooms-skeleton">
                    {[...Array(3)].map((_, i) => (
                      <div key={`room-skeleton-${i}`} className="room-card skeleton">
                        <div className="room-card-image" style={{ background: '#f3f4f6', height: '246px' }}>
                          <Skeleton height={246} />
                        </div>
                ...
                ) : !loading && hotelData && hotelData.rooms && processedRooms.length === 0 ? (
                  <div
            ...
            <div className="amenity-groups-container">
              ...
              >
                {loading && amenities.length === 0 ? (
                  <div className="amenity-skeleton-wrapper d-flex gap-4 w-100">
                    <div className="flex-grow-1 d-flex flex-column gap-4">
                      {[...Array(3)].map((_, i) => (
                        <div key={`amenity-sk-l-${i}`} className="amenity-group-card skeleton p-4 border rounded">
                    ...
                    <div className="flex-grow-1 d-flex flex-column gap-4">
                      {[...Array(2)].map((_, i) => (
                        <div key={`amenity-sk-r-${i}`} className="amenity-group-card skeleton p-4 border rounded">
                          <Skeleton width="35%" height={24} className="mb-3" />
                          <div className="d-flex flex-column gap-2">
                    ...
                    const renderChunk = (chunk: ChunkData) => {
                          ...
                          >
                            {chunk.codes.map((code) => (
                              ...
                              >
                                ...
                                >
                                  {groupedAmenities[code].map((facility: Facility) => (
                                    <div
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {amenitiesInGroup.map((facility: Facility) => (
                              <div
                                key={`${facility.facilityGroupCode}-${facility.facilityCode}-${facility.description.content}`}
                                className="amenity-item-box d-flex align-items-center"
                              >
